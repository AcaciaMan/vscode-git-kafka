...
export class ProviderDirsPaths implements vscode.WebviewViewProvider {
  ...
  constructor(private readonly context: vscode.ExtensionContext) {
    this.viewClicked = new ViewClicked(context);
  }
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    const htmlPath = webview.asWebviewUri(
      vscode.Uri.file(
        path.join(this.context.extensionPath, "resources", "htmlDirsPaths.html")
      )
    );
  ...
  private async _setDirsPath(sDirsPaths: {
    includeDirs: string;
    excludeDirs: string;
    pathSpec: string;
  }) {
  ...
  private _showClickedFiles() {
    this.viewClicked.showClickedFiles();
  }
}