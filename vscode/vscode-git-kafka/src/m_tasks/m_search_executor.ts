import { M_SolrDirsSearch, M_SolrSearch } from "../m_solr/m_solr_search";
...
export class M_SearchExecutor {
  ...
  mSolrDirsSearch: M_SolrDirsSearch;
  ...
  constructor(mTask: M_Task, mExecutor: TaskExecutor) {
    ...
    this.mSolrDirsSearch = new M_SolrDirsSearch(mTask, mExecutor.task.getId());
  }
...
export class M_SearchSolrDirs extends M_SearchExecutor {
  async executeSearch() {
    await this.mSolrDirsSearch.search();
  }
}